# codes adapted from https://github.com/Goedel-LM/Goedel-Prover.git
# all copyright to https://github.com/Goedel-LM/Goedel-Prover.git
...
use_lora  = len(args.peft_path) > 3
###############################################################################

###############################################################################
# 1. 모델 로드
###############################################################################
common_llm_kwargs = dict(                       #qwen:131072, deepseek:4096, internlm: 8192
...
#qwen:131072, deepseek:4096, internlm: 8192
if use_lora:
    llm = LLM(enable_lora=True, **common_llm_kwargs)
...
# 3. 추론
###############################################################################
extra_kwargs = dict(use_tqdm=True)
if use_lora:
    extra_kwargs["lora_request"] = lora_req
model_outputs = llm.generate( model_inputs,
    sampling_params,
    **extra_kwargs )
