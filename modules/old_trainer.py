...
from utils import CUSTOmConfig
...
class CUSTOMTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        for i in reversed(range(len(rewards_per_func.shape[-1]))):
            next_values = values[:, i + 1] if i < len(rewards_per_func.shape[-1]) - 1 else 0.0      #values=return in one trajectory environment
        ...
        reward_per_func = rewards_per_func.mean
        for i, reward_func in enumerate(self.reward_funcs):
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_logps = self._get_per_token_logps(model, completion_ids, completion_mask, logits_to_keep)

        # Compute the KL divergence between the model and the reference model
        per_token_kl = (
                torch.exp(ref_per_token_logps - per_token_logps)
        )       #do not need to subtrac, just add
        ...
        if 'ppo' in args.loss_function.lower():
        ...
        elif 'reinforce' in args.loss_function.lower():     #baseline?
            kl = ref_per_token_logps - per_token_logps
            kl_reward = args.kl_coef * kl
