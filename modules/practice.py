...
class CUSTOMTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                rewards_per_func = output_reward_func.to(dtype=torch.float32, device=device)  # rewarl
            ...
            else:  # Function as reward
                # Repeat all input columns (but "prompt" and "completion") to match the number of generations
                keys = [key for key in inputs[0] if key not in ["prompt", "completion"]]
                reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
                output_reward_func = reward_func(prompts=prompts, completions=completions,
                                                 **reward_kwargs)  # reward feedback generation, lean4_scheduler
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)

        # value estimation args gamma (self.args.gamma)
        rewards_per_func = gather(rewards_per_func)
        for i in reversed(range(len(rewards_per_func.shape[-1]))):
            next_values = values[:, i + 1] if i < len(rewards_per_func.shape[-1]) - 1 else 0.0
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        pg_loss = masked_mean(pg_loss_max, attention_mask)
        loss = pg_loss
