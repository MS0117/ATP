                    ...
                    if len(self._monitor_log) > 0:
                        print(
                            'TaskQueue-{}:  {} requests popped with avg batch_size {:.1f} in last period  {} waiting in queue'.format(
                                self.name, np.sum(self._monitor_log), np.mean(self._monitor_log),
...
class ProcessScheduler(object):
    ...
    def submit_request(self, data):
        with self.lock:
            ...
            self.task_queue.put((time.time(), request_id, data))            #by for in submit_all_request, in task_queue=[(time,id,Lean_code1),(time,id,Lean_code2)...]
        return request_id

    def submit_all_request(self, data_list):
        request_id_list = [self.submit_request(data) for data in data_list]     #data=[Lean_code1, Lean_code2],  => request_id_list=[id1,id2...]
    ...
    def get_all_request_outputs(self, request_id_list):                 #get_all_request_outputs([id1,id2,id3])
        outputs_list = []
        for request_id in request_id_list:
            outputs_list.append(self.get_request_outputs(request_id))   #get_all_request_outputs([id1,id2,id3])
        return outputs_list
