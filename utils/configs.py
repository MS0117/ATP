...
class DataArguments:
    ...
    dataset_name: Optional[str] = field(
        default=None,
        metadata={"help": ("Dataset name in local path or HuggingFace repo")},
    )
    ...
    use_test_set: bool = field(
        default=False,
        metadata={"help": "Configure to evaluate after/during the training"}
    )
    # Logging
...
class PPOConfig(trl.PPOConfig):
    """
    Arguments related to the PPO training process itself. For all parameters, see: https://huggingface.co/docs/transformers/v4.39.3/en/main_classes/trainer#transformers.TrainingArguments
    """
...
@dataclass
class CUSTOMConfig(trl.GRPOConfig):
    ...
    optim: Optional[str] = field(default="rmsprop")
    remove_unused_columns: bool = field(default=False)
    loss_function: Optional[str]=field(default="rloo")
    rloo_token_level: Optional[bool]=field(default=True)
    kl_coef: Optional[float]=field(default=0.9)
    reward_type: Optional[str]=field(default=None)
    normalize_advantage :Optional[bool]=field(default=True)
    num_generation : Optional[int]=field(default=16)
    num_iterations : Optional[int]=field(default=4)
